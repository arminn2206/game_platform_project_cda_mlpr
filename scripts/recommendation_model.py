...
if __name__ == "__main__":
    ...
    evaluate_model_holdout(interactions, df_games_clean, features, knn)import sqlite3
import pandas as pd
import os
import numpy as np
import matplotlib.pyplot as plt
...
def evaluate_model_holdout(interactions, df_games, feature_matrix, knn_model):
    ...
    for N in n_cutoffs:
        ...
        for user in sample_test_users:
            ...
            recommended_indices = indices[0]
            recs = feature_matrix.iloc[recommended_indices]['game_id'].values
            recs_filtered = [r for r in recs if r not in known_likes][:N]
            
            hits = len(set(recs_filtered).intersection(hidden_likes))
            
            precision = hits / N if N > 0 else 0
            recall = hits / len(hidden_likes) if len(hidden_likes) > 0 else 0
